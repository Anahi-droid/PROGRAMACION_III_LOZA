...
function updateScale(newX) {
   ...
   scaleValue = Math.max(0.5, Math.min(2.5, scaleValue));
   insignia.style.transform = `scale(${scaleValue})`;
   startX = newX;
}

insignia.addEventListener('mousedown', (e) => {
   e.preventDefault();
   isDragging = true;
   startX = e.clientX;
});
...
insignia.addEventListener('touchstart', (e) => {
   e.preventDefault();
   isDragging = true;
